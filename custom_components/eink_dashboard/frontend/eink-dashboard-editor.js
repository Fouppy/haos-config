 ...
 * @returns A single ha-form schema entry.
 */
function iconStyleSelector(defaultStyle = DEFAULT_ICON_STYLE) {
    return {
        name: "icon_style",
...
export const HELPERS = {
    show_all: "When off, the widget is empty if no collection falls "
...
export async function loadHaComponents() {
    ...
    if (!customElements.get("ha-card-conditions-editor")) {
        ...
        const cls = customElements.get("hui-conditional-card");
        if (cls)
            await cls.getConfigElement();
    }
}
...
export function getSummary(widget) {
    ...
    if (t === "entities") {
        ...
        const count = (widget.entities || []).filter((r) => typeof r === "string" || ("entity" in r && !("type" in r))).length;
...
// ── Icon paths (MDI) ─────────────────────────────────────────────
const MDI_DRAG = "M7,19V17H9V19H7M11,19V17H13V19H11"
...
const MDI_CHEVRON_DOWN = "M7.41,8.58L12,13.17L16.59,8.58"
    + "L18,10L12,16L6,10L7.41,8.58Z";
const MDI_CLOSE = "M19,6.41L17.59,5L12,10.59L6.41,5"
...
class EinkDashboardEditor extends HTMLElement {
    ...
    async _buildShell() {
        ...
        const size = `${this._display.width} × ${this._display.height}`;
    ...
    `;
        ...
        this.shadowRoot
            ...
            .addEventListener("item-moved", ((ev) => {
    ...
    _buildWidgetItem(widget, index) {
        ...
        handle.addEventListener("keydown", (ev) => this._onHandleKeyDown(ev, index));
    ...
    _buildWidgetForm(widget, index) {
        ...
        form.addEventListener("value-changed", ((ev) => {
            ...
            const prevEntity = this._widgets[index].entity;
            const data = {
                type: widget.type,
                ...raw,
            };
            ...
            if (widget.type === "waste_schedule"
                ...
                this._refreshEntriesEditor(container, index);
            }
        }));
     ...
     */
    _refreshEntriesEditor(container, index) {
        const old = container.querySelector(".entries-section");
        if (old)
            old.remove();
        this._buildEntriesEditor(container, index);
    }
     ...
     * @param index     - Widget index in the widget array.
     */
    _buildEntriesEditor(container, index) {
        ...
        const attrKeys = Object.keys(attrs).filter((k) => !EinkDashboardEditor._HA_INTERNAL_ATTRS.has(k));
        ...
        for (const key of attrKeys) {
            ...
            labelInput.addEventListener("input", syncEntries);
    ...
    _updateSummary(index) {
        const item = this.shadowRoot
            .querySelector(`.widget-item[data-index="${index}"]`);
        if (!item)
            return;
        const label = item.querySelector(".widget-label");
        if (label) {
    ...
    _onDelete(index) {
        ...
        else if (this._sortSelectedIndex !== undefined
    ...
    _onHandleKeyDown(ev, index) {
        ...
        if (this._sortSelectedIndex === undefined)
            return;
        ...
        requestAnimationFrame(() => {
            const item = this.shadowRoot
                .querySelector(`.widget-item[data-index="${newIndex}"]`);
            item
     ...
     */
    _applySortSelectedClass() {
        this.shadowRoot
            .querySelectorAll(".widget-item")
            .forEach((el) => {
            const idx = parseInt(el.dataset.index ?? "", 10);
            el.classList.toggle("sort-selected", idx === this._sortSelectedIndex);
        });
    }
    /**
     ...
     */
    _onAddBtnClick() {
        let picker = this.shadowRoot.querySelector("eink-widget-picker");
        if (!picker) {
            picker = document.createElement("eink-widget-picker");
            picker.addEventListener("type-selected", ((ev) => {
...
EinkDashboardEditor._HA_INTERNAL_ATTRS = new Set([
    "friendly_name",
    "unit_of_measurement",
    "icon",
    "device_class",
